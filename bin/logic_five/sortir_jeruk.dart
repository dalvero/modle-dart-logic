...
void main(){
  // INPUT
  stdout.write("Masukan banyaknya jeruk\t: ");
  int? n = int.tryParse(stdin.readLineSync() ?? '');

  if (n != null && n > 0 && n < 200) {
    List<int> datasJeruk = [];
    for (var i = 0; i < n; i++) {
      stdout.write("Kualitas jeruk ke-${i + 1}: ");
      int? kualitas = int.tryParse(stdin.readLineSync() ?? '');
      if (kualitas != null && kualitas >= 0 && kualitas < 101) {
        datasJeruk.add(kualitas);
      } else {
        print("Kualitas diluar jangkauan, silahkan ulangi!");
        return;
      }
    }
    ...
    for (var i = 0; i < datasJeruk.length; i++) {
      if (datasJeruk[i] >= 85 && datasJeruk[i] <= 100) {
        e++;
      } else if (datasJeruk[i] >= 60 && datasJeruk[i] <= 84){
        l++;
      } else if (datasJeruk[i] >= 0 && datasJeruk[i] < 60){
        r++;
      }
    }
